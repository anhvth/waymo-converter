...
def extract_tf_file(filename):
    print('extract_tf_file:',filename)
    ...
    for frameno, frame in tqdm(enumerate(datafile)):
        ...
        display_laser_on_image(img, pcl, pcl_attr,vehicle_to_image)

        # Display the label's 3D bounding box on the image.
        display_labels_on_image(camera_calibration, img,
                                frame.laser_labels, visibility)

        # Display the image
        # cv2.imshow("Image", img)
        cv2.imwrite(f'show-img-{frameno}.jpg', img)
        # cv2.waitKey(10)
